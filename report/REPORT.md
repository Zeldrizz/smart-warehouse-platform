...
**Почему пункт закрыт полностью.** В ТЗ дано право самому спроектировать структуру jobs и stages. Здесь структура не случайна: `build-images` отделён, чтобы ранний build failure не тратил время на тестовую среду; `unit-tests` вынесен отдельно, чтобы дешёвые ошибки отсекались раньше дорогого Docker stack; `start-stack` объединяет stack-dependent шаги в один job, потому что GitHub-hosted jobs не делят живой Docker daemon state между собой. Важно и то, что pipeline проверяет систему на нескольких уровнях строго по возрастанию стоимости: сначала собираем образы, потом проверяем локальную логику сервисов, потом поднимаем тяжёлый compose-стенд, затем проверяем взаимодействие, сквозной сценарий, нагрузку и finally метрики качества. Это не "минимальный YAML для галочки", а осмысленный CI design под ограничения платформы и под саму архитектуру решения.
...
- `wms_service/tests/unit/test_event_publisher.py:1-114`
- `wms_service/tests/unit/test_http_metrics.py:1-92`
- `consumer_service/tests/unit/test_event_processor.py:1-347`
...
**Почему пункт закрыт полностью.** Тесты не проверяют один handler в изоляции, а проходят через настоящие transport/storage boundaries: HTTP, Kafka, Cassandra, Prometheus. Для этого используется отдельный `tests` container, который подключён к той же сети и тем же сервисам, что и production-like стек из compose. Это принципиально важно: ошибка сериализации события, проблема с topic routing, с consumer group, с Cassandra persistence или с Prometheus scrape не проявилась бы в чистом unit-слое, но проявляется здесь. То есть integration suite действительно валидирует межсервисные контракты, а не переименовывает набор локальных тестов в "integration".

**Где смотреть.**

- `tests/integration/test_service_interactions.py:1-219`
...
- `test_posted_events_increment_wms_and_consumer_metric_series` проверяет, что реальный `POST /api/v1/events` увеличивает и WMS HTTP metrics, и consumer-side processing metrics.

**Почему этот набор сценариев достаточен.** Он покрывает четыре разных класса интеграции: happy-path межсервисный поток, failure-path с DLQ, observability-path через `/metrics` + Prometheus scrape и причинную связь между реальным API-вызовом и ростом metric series. То есть проверяется не один случай "событие дошло", а четыре независимых межкомпонентных контракта. Последний сценарий особенно полезен для защиты: он доказывает, что observability не живёт отдельно от бизнес-функциональности, а действительно отражает реальные действия системы.
...
- `tests/integration/test_service_interactions.py:143-213`
...
**Почему пункт закрыт полностью.** E2E здесь соответствует буквальной формулировке задания: начинается с публичного API, проходит через реальный event pipeline и заканчивается в конечном хранилище. Это не integration test, переименованный в E2E, а действительно сквозной пользовательский поток. Важна и предметная насыщенность сценария: он охватывает не одну примитивную операцию, а последовательность складских действий, после которой меняются и inventory projection, и product totals, и order state. За счёт этого E2E доказывает корректность не отдельного шага, а жизненного цикла доменного процесса.
...
**Почему пункт закрыт полностью.** Важен не только сам факт наличия трёх метрик, но и способ их сбора. Здесь сделан единый middleware-слой, который автоматически записывает запросы для обоих сервисов в одинаковом формате и с одинаковым набором labels. Это соответствует промышленному подходу и снимает риск расхождения между сервисами. Такой дизайн ещё и облегчает дальнейшее использование метрик: Grafana, Prometheus gates и alert rules могут работать с единым семейством метрик для обоих приложений, не требуя специальных исключений для каждого сервиса.
...
**Почему пункт закрыт полностью.** Dashboard не ограничивается одной красивой картинкой. Он закрывает обе стороны системы: входной HTTP producer (`wms-service`) и асинхронный consumer (`consumer-service`). При этом дашборд provisioned из JSON, а не создан вручную в локальной Grafana, что важно для воспроизводимости в CI и на защите. Отдельно важно, что producer и consumer видны не на разрозненных страницах, а в одном operational контексте: так проще объяснять причинно-следственные связи между входной нагрузкой, последующей обработкой и состоянием очереди.
...
**Почему это сделано именно так.** ТЗ допускает либо отдельные дашборды, либо явно выделенные секции. Здесь выбран второй вариант, потому что для event-driven архитектуры особенно полезно видеть producer-path и consumer-path рядом. Тогда на одном экране можно показать, как сначала растёт WMS throughput, потом consumer throughput, затем меняются processing latency и end-to-end delay. Это лучше подходит для диагностики и для защиты, чем переключение между несколькими экранами.
...
**Почему это важно.** Реальное обновление в UI означает, что графики строятся не по статически импортированным данным и не по заранее сохранённым snapshot-ам. На защите можно дать нагрузку или запустить demo-сценарий и показать, как линии меняются прямо в процессе. Это сильное доказательство того, что observability-контур живой.
...
**Почему пункт закрыт полностью.** Инфраструктурный dashboard не дублирует сервисный. Он отвечает на другой вопрос: не "что ответил API", а "где bottleneck в платформе". Для этого объединены Kafka-exporter метрики и container-level metrics через cAdvisor. Благодаря такому разделению сервисные панели показывают симптом на уровне поведения системы, а инфраструктурные панели помогают перейти к гипотезе о причине: очередь, broker, Cassandra resource pressure или недоступность monitoring target.
...
**Почему этот набор действительно отвечает на вопрос про узкое место.** Если растёт WMS latency, но lag и consumer-метрики нормальные, проблема ближе к входному API или producer-path. Если WMS отвечает быстро, а lag и end-to-end delay растут, узкое место уже после API, в Kafka/consumer/storage path. Если одновременно растут lag и resource metrics Cassandra/Kafka, можно уже обосновывать инфраструктурную причину. То есть панели дают не просто набор чисел, а диагностическую схему локализации проблемы.
...
**Почему пункт закрыт полностью.** Нагрузка не существует отдельно от CI и не запускается как локальная "демка". Она встроена в pipeline после проверки функциональной корректности, то есть система сначала доказывает корректность, а затем выдерживает controlled load и только после этого проходит SLI gates. Это важный порядок: нет смысла интерпретировать метрики производительности до того, как подтверждено, что система вообще обрабатывает сценарии корректно.
...
**Почему именно такой сценарий считается реалистичным и уместным для CI.** Для учебного CI важно дать достаточно трафика, чтобы появились meaningful series в Prometheus и чтобы проявились проблемы с producer/consumer path, но при этом не превратить прогон в слишком долгий stress test. `10 VUs` на `30s` решают именно эту задачу: нагрузка уже не игрушечная, но всё ещё дешева по времени и ресурсам для GitHub Actions. Отдельный health scenario нужен, чтобы параллельно проверять не только latency основного API, но и то, что сервис остаётся доступным под нагрузкой.
...
**Почему это важно.** Здесь метрики собираются не "после теста", а параллельно с жизнью стенда. За счёт этого gates анализируют реальное поведение системы под нагрузкой и во время E2E-прогона, а не искусственно собранный постфактум summary. Это существенно ближе к реальной эксплуатации, где Prometheus наблюдает систему во времени, а не вычисляет показатели из логов после завершения работы.
...
**Почему пункт закрыт полностью.** Здесь есть все три обязательные части: сами правила как YAML в репозитории, Alertmanager в compose и отдельный demo-сценарий, который переводит алерты в `firing`. Без третьей части это был бы только "конфиг на бумаге"; здесь же срабатывание реально воспроизводится. Кроме того, набор правил покрывает разные режимы отказа системы: деградацию пользовательского входа, деградацию асинхронной обработки и прямую недоступность компонентов. Это делает alerting содержательным, а не формальным.
...
**Почему пункт закрыт полностью.** Все три SLI относятся именно к системе в целом, а не к отдельному внутреннему методу. Первый описывает доступность публичного входа, второй - качество producer-path, третий - freshness асинхронной обработки и проекций. Вместе они покрывают вход, транспорт и конечную наблюдаемую полезность системы. Важно и то, что они не дублируют друг друга. `API availability` отвечает на вопрос "можно ли прямо сейчас отдать работу системе", `WMS publish latency p95` - "насколько быстро эта работа принимается системой в синхронном producer-path", а `Event end-to-end delay p95` - "насколько быстро событие становится полезным конечным состоянием". Именно такое разделение хорошо соответствует event-driven архитектуре склада.
...
**Почему выбраны именно такие значения.** `API availability` жёстко привязан к тому, что `wms-service` является публичным входом системы. Пользователь не видит Kafka и Cassandra напрямую, он сначала должен успешно сдать событие на вход, поэтому для этого пути обоснован высокий целевой уровень `99.5%`. Порог `95%` используется уже как operational failure threshold: ниже него у системы слишком много отказов именно в пользовательской точке входа, и сборка должна считаться неприемлемой.

Для `WMS latency p95` выбран именно percentile, а не среднее значение, потому что среднее легко скрывает плохие хвосты. В producer-path `wms-service` не делает тяжёлой синхронной обработки: он валидирует входной payload и публикует событие в Kafka. Поэтому target `<= 0.5s` реалистичен для здорового контура, а `<= 1.0s` уже выступает как граница заметной деградации входного API.

Для `Event end-to-end delay p95` бюджет шире намеренно. Эта метрика описывает не скорость ответа API, а полный жизненный цикл события: Kafka delivery, polling consumer-а, прикладную обработку и обновление проекций в Cassandra. Для такого асинхронного пути целевой бюджет `<= 5.0s` выглядит разумным, а `<= 10.0s` - это уже operational failure threshold, после которого данные становятся слишком несвежими, чтобы считать систему качественно работающей.
...
**Почему пункт закрыт полностью.** Это важно не только технически, но и методологически. SLI не вычисляются из "служебных переменных" тестового скрипта и не задаются вручную после прогона, а берутся из тех же time series, которые реально живут в Prometheus и визуализируются в Grafana. За счёт этого любой проверяющий может открыть Prometheus, выполнить тот же запрос и получить то же значение. Значит, SLI встроены в реальный мониторинговый контур системы, а не существуют как отдельная искусственная логика в CI.
...
**Почему это важно.** Здесь SLI не являются пассивной документацией. Они участвуют в принятии автоматического решения о качестве сборки после нагрузки. То есть observability-контур влияет на outcome pipeline так же, как unit или integration tests. Это очень сильное закрытие требования "показать, как SLI используются": они не просто описаны, а реально встроены в процесс контроля качества.
...
**Почему это важно для максимальной оценки.** Здесь выполнено не только техническое условие наличия SLI, но и документирование инженерного смысла порогов. За счёт этого на защите можно объяснить не просто "какой threshold стоит", а почему он выбран именно таким, какой кусок архитектуры он отражает и что operationally считается деградацией. Фактически README превращается из "инструкции по запуску" ещё и в краткий эксплуатационный контракт системы.
...
- `INVENTORY_COUNTED`;
- `PRODUCT_MOVED`;
- invalid move `ZONE-A -> ZONE-A` с уходом в DLQ;
- request -> event mapping для `v1` и `v2`;
- generated publish path: приватный `_routing_key` и запрет протечки `_...` полей в payload;
- middleware labels для `503`, `422` и `500 / unhandled_exception`.

Почему это важно. Набор unit-тестов больше не ограничивается happy path-ами. Он проверяет и доменные переходы, и защитные ветки обработки, и observability-инструментацию. За счёт этого unit-слой действительно страхует те куски логики, которые дешевле и надёжнее всего проверять без поднятия всего стека: маппинг request -> envelope, идемпотентность, stale-paths, перерасчёт остатков, реакции на неправильные события и корректную разметку метрик.

Где смотреть:

- `consumer_service/tests/unit/test_event_processor.py:71-347`
- `wms_service/tests/unit/test_event_publisher.py:21-114`
- `wms_service/tests/unit/test_http_metrics.py:17-92`
...
Почему это важно. Графики удобны для человека, но они не дают формального критерия качества сборки. Отдельный gate через Prometheus API превращает observability из режима "визуально похоже на норму" в режим воспроизводимого автоматического решения. За счёт этого pipeline опирается не на устную интерпретацию графиков, а на конкретные PromQL-запросы и зафиксированные thresholds.
...
Почему это важно. Во многих учебных работах alert rules существуют только в YAML и никогда не доводятся до реального `firing`. Здесь отдельно демонстрируется, что правила действительно срабатывают в живом стенде, а не только выглядят правдоподобно на бумаге. Это резко усиливает доказательную базу на защите.
...
- рост WMS и consumer metric series после реального `POST /api/v1/events`
